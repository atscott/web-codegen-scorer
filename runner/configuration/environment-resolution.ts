...
import {assertIsEnvironmentConfig, isLocalEnvironmentConfig} from './environment-config.js';
...
export async function getEnvironmentByPath(
  ...
  const environment = isLocalEnvironmentConfig(result.default)
    ? new LocalEnvironment(rootPath, result.default, await getRunnerByName(runnerCliOption))
