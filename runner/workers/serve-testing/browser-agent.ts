...
export async function runBrowserAgentUserJourneyTests(
  ...
  const tmpDir = await mkdtemp(join(tmpdir(), 'browser-agent-user-journey-task-'));
  const taskJsonFile = join(tmpDir, 'task.json');
  await writeFile(taskJsonFile, JSON.stringify(agentTask));

  const [hasPython, hasUv] = await Promise.all([binaryExists('python3'), binaryExists('uv')]);
  ...
  try {
    ...
    progressLog('error', 'Error while running user journey browser tests', e + '');
    return null;
  }
}
