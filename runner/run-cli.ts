...
import {BUILT_IN_ENVIRONMENTS, LLM_OUTPUT_DIR} from './configuration/constants.js';
...
async function runApp(options: Options) {
  ...
  const {directory, cleanup} = await setupProjectStructure(environment, rootPromptDef, progress);
